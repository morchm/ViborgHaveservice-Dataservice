...
export default function reviewsEdit({searchParams}:{searchParams:{id:string}}){

    const id = searchParams.id  

    // --- useRequest
    const { data, isLoading, error, makeRequest } = useRequestData<Review>();
    const {data: dataPUT, isLoading: isLoadingPUT, error: errorPUT, makeRequest: makeRequestPUT } = useRequestData<ReviewPOST>();
    ...
    const handleSubmit = (e:FormEvent) => {
        ...
        makeRequestPUT("http://localhost:5023/reviews/admin/" + id, "PUT", newReview);
    };
    ...
    return (
        <div className="max-w-screen-2xl bg-neutral-100 h-screen">
            <Navbar/>
            <div className="p-10">
                <Link href="/reviewsAdmin" className="mainGreenBackground text-white hover:bg-green-600 transition-colors p-3 rounded my-8">Tilbage</Link>
                ...
                <form onSubmit={handleSubmit} className="text-center">
                    ...
                    <button type="submit" className="mainGreenBackground text-white hover:bg-green-600 transition-colors p-3 rounded my-8">Ret anmeldelse</button>
                </form>
            </div>
        </div>
    )
